...
<%
	...
	if (flagLog){
		...
		se.setAttribute("path", "");
		User user = otherService.getUser(request.getRemoteUser());
		se.setAttribute("user", user);
		
		if ("client".equals(user.getRole())){
			Client cl = clientService.getUser(request.getRemoteUser());
			se.setAttribute("path", cl.getRoot());
			user = cl;
			
			//System.out.println("Dosao sa adrese: " + request.getRemoteAddr() + ", Dozvoljeno sa: " + cl.getDomen());
			
			if (!request.getRemoteAddr().equals(cl.getDomen())){
				se.invalidate();
			}else{
				se.setAttribute("user", user);
			}
		}else if ("adminD".equals(user.getRole())){
			AdminD ad = adminDService.getUser(request.getRemoteUser());
			se.setAttribute("path", ad.getRoot());
			user = ad;
			se.setAttribute("user", user);
		}
